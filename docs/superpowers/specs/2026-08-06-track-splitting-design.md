...
## 구현 중 발견 — 지하철 `leg_order` 의미 충돌 (미해결, 별도 작업)

백필 후 지하철 매칭을 다시 돌리자 187일 중 **12일이 실패**했다. 매치 155건 중 47건이
`session_id`를 받지 못했다. 지하철 탐지와 라벨링 자체는 정상이다 — 세그먼트 121개가
`subway`로 라벨링됐고, 빠진 것은 환승 세션 묶음뿐이다.

원인은 순서 문제가 아니라 설계 충돌이다. `leg_order`가 두 가지 뜻을 갖는다:

- `matcher.ts` — 세그먼트 안에서의 순번. 한 세그먼트가 Case A로 쪼개지면 0, 1을 넣는다
- `session-grouper.ts` — 세션 안에서의 순번으로 덮어쓴다

그런데 유니크 제약은 `UNIQUE(transportation_segment_id, leg_order)`다. 한 세그먼트의 두
leg가 서로 다른 세션에 들어가면 둘 다 `leg_order = 0`을 노려 유니크 위반이 나고,
트랜잭션이 통째로 롤백되어 그날 세션 배정이 사라진다.

두 leg는 시간상 인접하므로 같은 세션에 묶이는 것이 정상이지만, 분할 지점이 두 노선을 모두
가진 역 근처가 아니면 `sameInterchange`가 거짓이 되어 세션이 갈린다. 이때 뒤쪽 leg는 구조상
반드시 새 그룹의 0번이 된다(그룹 분리 시 `current = [row]`).

커밋 `b75f0b9`의 2단계 파킹(음수로 먼저 옮긴 뒤 최종 값 쓰기)은 **중간 상태** 충돌만
제거한다. 위 케이스는 **최종 값**이 충돌하므로 막지 못한다 — 실측으로 12일 전부 여전히
실패함을 확인했다. 그 커밋은 별개의 충돌 유형을 제거하고 테스트가 붙어 있어 남겨두었다.

해결하려면 `leg_order`의 의미를 먼저 확정해야 한다:

- 유니크 키를 `(session_id, leg_order)`로 바꾸고(마이그레이션), 매처의 세그먼트 단위 중복
  방지를 대체할 제약을 설계한다
- 또는 그룹화가 `leg_order`를 쓰지 않고, 세션 내 순서를 읽을 때 `sub_start_time`으로
  파생한다. 이 경우 `usage.ts`가 `leg_order > 0`으로 환승을 세는 부분을 다시 짜야 한다
