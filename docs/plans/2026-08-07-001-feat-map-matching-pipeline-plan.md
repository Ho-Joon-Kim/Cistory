...
| Valhalla container down | ~~Every segment that day records `failed`, and nothing retries it automatically~~ Superseded by `fix/valhalla-unreachable`: the adapter now distinguishes "could not reach the engine" (`ValhallaUnreachableError`) from "the engine responded" (a `MatchResult` status). `matchRoutesForDay` lets that propagate and writes nothing for the day rather than recording `failed` everywhere, so R1's "no row = unprocessed" invariant holds and the day stays a retry candidate | KTD3 still keeps the day itself completing and `/overview` finalising. Recovery is automatic, not deliberate: the existing hourly catch-up always re-includes "yesterday"/"today" regardless of completion status, so it retries the day's route matching on its own once the engine responds — no operator action, no manual `scripts/backfill-route-matches.ts` run needed for this case. That script still exists for the genuinely manual cases (widened extracts, reclassified segments) and now fails loudly instead of silently no-op'ing if it hits an unreachable engine mid-run |
