 ...
 * flow. Cloned from `withings-oauth-state.ts` (the plan defers unifying the two).
 *
 ...
 * purpose context — DISTINCT from the Withings one so the two flows can't
 * cross-verify each other's state tokens, and so a key leak does not let an
 * attacker BOTH decrypt stored tokens AND forge state (AES-GCM uses a different salt).
 */

const STATE_TTL_MS = 10 * 60 * 1000; // 10 minutes
const STATE_CONTEXT = "google-health-oauth-state";
